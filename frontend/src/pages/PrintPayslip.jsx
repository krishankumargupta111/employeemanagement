...
api.get(`payslips/${id}`).then((res)=>setPayslip(res.data)).catch(console.error).finally(()=>setLoading(false))
  ...
  if(!payslip) return <p className='text-center py-12 text-slate-400
  '>Payslip not found</p>
    return (
    <div className='max-w-2xl mx-auto p-8 bg-white animate-fade-in'>
      ...
      mb-8'>
        ...
        '>PAYSLIP</h1>
        <p className='text-slate-500 text-sm mt-1'>{format(new Date (payslip.year,payslip.month-1),
        "MMMM yyyy")}</p>
      </div>
      <div className='grid grid-cols-2 gap-6 mb-8'>
        <div>
          <p className='text-xs text-slate-400 uppercase tracking-wider
          mb-1'>Employee Name</p>
          <p className='font-semibold text-slate-900'>{payslip.employee?.firstName} {payslip.employee?.lastName}
</p>
        </div>
...
<div>
          ...
          mb-1'>Period</p>
          <p className='font-semibold text-slate-900'>
            {format(new Date (payslip.year,payslip.month-1),
        "MMMM yyyy")}</p>
</div>
...
<td className='text-right py-4 px-4 text-slate-900 font-medium
text-lg'>
