...
function Leave() {
  ...
  const fetchLeaves = useCallback(async() => {
    try{const res=await api.get('/leave')
...
toast.error(error?.response?.data?.error ||error.message)
    ...
    <div className='animate-fade-in'>
      ...
      items-start sm:items-center gap-4 mb-8'>
        <div>
          ...
          <p className='page-subtitle'>{isAdmin ? "Manage Leave application" : "Your Leave history and requestes"}</p>
        </div>
        {!isAdmin && !isDeleted && (
  ...
  items-center gap-4 relative overflow-hidden group'>
    ...
    group-hover:bg-indigo-50 transition-colors duration-200'>
                ...
                <s.icon className='
    w-5 h-5 text-slate-600 group-hover: text-indigo-600
      transition-colors duration-200' />
              ...
              <div>
                ...
                tracking-tight
                '>{s.value} <span className='text-sm
                font-normal text-slate-400'>taken</span></p>
              </div>
...
<LeaveHistory leaves={leaves} isAdmin={isAdmin} onUpdate={fetchLeaves}/>
    <ApplyLeaveModal open={showModel} onClose={()=>setShowModel(false)}
