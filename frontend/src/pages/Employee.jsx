...
onChange={(e)=>setSearch(e.target.value)} value={search}/>
        ...
        <select value={selectedDept} onChange={(e)=>
          setSelectedDept(e.target.value)} className='max-w-40'>
<option value="">
  All Departments
</option>
...
{filtered.length===0 ?(
  ...
  bg-white rounded-2xl border border-dashed border-slate-200'>
    No Employees found</p>):(filtered.map((emp)=>(
...
{showCreateModel && (
  <div className='fixed bg-black/40 backdrop-blur-sm
  inset-0 z-50 flex items-start justify-center p-4 overflow-y-auto'
