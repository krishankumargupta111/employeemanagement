        ...
        return(
            ...
            p-8 bg-slate-50 rounded-2xl border border-slate-200'>
                <h3 className='text-lg font-bold text-slate-900'>
                    Work Day Completed</h3>
                <p className='text-slate-500 text-sm mt-1'>
                    Great job! see you tomorrow</p>
            </div>
        )
    }
    const isCkeckedIn=!!todayRecord?.checkIn;
  ...
  return (
    ...
    <div className='absolute bottom-4 right-4 flex flex-col z-1'>
        ...
        className={`w-full max-w-xs flex justify-between
            items-center gap-8 p-4 rounded-xl bg-linear-to-br
            text-white ${isCkeckedIn ?"from-slate-700 to-slate-900"
                :"from-indigo-600 to-indigo-700"
            }`}>
            {loading ?<Loader2Icon className='size-7 animate-spin'/>:
            isCkeckedIn?<LogOutIcon className='size-7'/> 
            :<LogInIcon className='size-7'/>}
            <div className='relative flex flex-col items-center text-center'>
                <h2 className='
                text-lg font-medium mb-1'>{loading?"Processing...":isCkeckedIn?"Clock Out"
                :"Clock In"}
                </h2>
                <p className='text-xs opacity-80'>
                    {isCkeckedIn?"Click to end your shift"
