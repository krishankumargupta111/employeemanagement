...
function AttendanceStats({history}) {
    const totalPresent=history.filter((h)=>h.status==="PRESENT" |
h.status==='LATE').length;
  ...
  return (
    <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-5 mb-8'>
      {stats.map((s)=>(
        ...
        sm:p-6 flex items-center gap-4 relative overflow-hidden
        group'>
            <div className='absolute left-0 top-0 bottom-0
            w-1 rondeed-r-full bg-slate-500/70 group-hover
            :bg-indigo-500/70'/>
            <div className='p-3 bg-slate-100 rounded-lg group-hover
            :bg-indigo-50 transition-colors duration-200'>
