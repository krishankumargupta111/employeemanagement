...
placeholder='Write a brief bio...' className={`resize-none ${
  initialData.isDeleted ? "bg-slate-50 text-slate-400 cursor-not-allowed"
:""}`}/>
<p className='text-xs text-slate-400 mt-1.5'>
  This will be dispalyed on your profile.</p>
  </div>
  {initialData.isDeleted ?(
    ...
    <div className='flex justify-end pt-2'>
      ...
      >{loading?<Loader2Icon className='w-4 h-4 animate-spin'/>:<Save
      className='w-4 h-4'/>}Save Changes</button>
    </div>
  )}
</div>
    </form>
