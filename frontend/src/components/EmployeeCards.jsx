...
function EmployeeCards({ employee, onDelete, onEdit }) {
...
<div className='w-20 h-20 rounded-full bg-linear-to-br from-indigo-100
...
<div className='absolute top-3 left-3 flex gap-2'>
    ...
    backdrop-blur-sm px-2.5 py-1 text-xs
    font-semibold text-slate-600 rounded-lg shadow-sm'>
...
{!employee.isDeleted && (
    ...
    justify-center pb-6 gap-3'>
        ...
        text-slate-700 hover:text-indigo-600 rounded-xl
        shadow-lg transition-all hover:scale-105'>
            <PencilIcon className='w-4 h-4'/>
        </button>
        <button onClick={handleDelete} className='p-2.5 bg-white/90
        backdrop-blur-sm text-slate-700 hover:text-rose-600
